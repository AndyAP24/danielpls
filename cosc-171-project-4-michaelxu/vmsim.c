...
swap_out () {
  ...
  vmsim_addr_t real_page_addr; //to return

  pt_entry_t current_PTE; //for clock algorithm, the PTE we are trying to evict
  vmsim_read_real(&current_PTE, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  while (true) {
    CLEAR_REFERENCED(current_PTE);
    vmsim_write_real(&current_PTE, reverse_page_map[clock_hand], sizeof(pt_entry_t));
    clock_hand = (clock_hand+1) % normal_real_pages;
    vmsim_read_real(&current_PTE, reverse_page_map[clock_hand], sizeof(pt_entry_t));
    if (IS_REFERENCED(current_PTE) != 1 && IS_RESIDENT(current_PTE) == 1) {
      break;
    } 
  }
  // while (IS_REFERENCED(current_PTE) != 1) {
  //   CLEAR_REFERENCED(current_PTE);
  //   vmsim_write_real(&current_PTE, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  //   clock_hand = (clock_hand+1) % normal_real_pages;
  //   vmsim_read_real(&current_PTE, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  // }
  CLEAR_RESIDENT(current_PTE);
  real_page_addr = (vmsim_addr_t) GET_PAGE_ADDR(current_PTE);
  bs_write(GET_PAGE_ADDR(current_PTE), bs_free_block);
  current_PTE = bs_free_block << 1;
  vmsim_write_real(&current_PTE, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  bs_free_block++;

  //We want to find the lower page table entry using the Clock algorithm that we want to change, in effect evicting
  //the page. We want to replace the LPT entry which used to be the page address of the page we are evicting, and change it to
  //the block number that we are evicting to. However, this block number last bit still will be viewed as a resident bit, so we 
  //should use the last 31 bits as the actual number, and the first bit as a resident bit of 0. 
  // There is a variable which tells us where the next free block in the backing storage is, and we will evict the data
  //of the page into this next free block.
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  bs_read(real_page_addr, pte>>1);
  


  // COMPLETE ME: Read a page from the backing store into the given real page.
  //We want to put this real_page_addr into an entry in a LPT. We want to make sure this address has 1 as its resident bit (the first bit)
