...
swap_out () {
  ...
  // Select a real page to swap out. If we are swapping out page #N, reverse_page_map[N] will have the corresponding PTE.
  // Selection algorithm to do so - clock 
  // Use reverse_page_map array
  // Get data from page and pass the data into the backing store
  // Write that page's 4kb of data to the backing store at block bs_free_block

  // Make a note of the fact that the block number bs_free_block corresponds to this page that we just swapped out.
  // Doing this in the page table entry!
  // Increment bs_free_block

  // Make sure to set the resident etc bits for the pte that weve chosen and swapped out.

  // CLOCK ALGORITHM:
  // while (victim page not found):
  //    use clock hand to select pte
  //    read pte from page table
  // 
  //    if (current page REFERENCED):
  //       "Reset used bit":
  //       set REFERENCED=0
  //       write pte back to page table
  //    else:
  //       "Replace current page":
  //       write current page data to backing storage
  //       increment backing storage free idx
  //       store backing storage idx in pte
  //       write pte back to page table
  //
  ...
  while (true) {
    // Select our victim page 
    vmsim_addr_t pte_addr = reverse_page_map[clock_hand];
    vmsim_addr_t pte;
    vmsim_read_real(&pte, pte_addr, sizeof(pte));

    if(IS_REFERENCED(pte)) {
      CLEAR_REFERENCED(pte);
    } else {
      vmsim_addr_t real_page_addr = GET_PAGE_ADDR(pte);
      bs_write(real_page_addr,bs_free_block);
      pte = bs_free_block << 1;
      bs_free_block++;
    }

    // Write changes to pte
    vmsim_write_real(&pte, pte_addr, sizeof(pte));

    // Advance clock pointer
    clock_hand++;
    clock_hand %= normal_real_pages;
  }
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  // We now have a page table entry that was not RESIDENT
  // and a spot in real memory where we can put the data from the bs

  // So we derive the block number we stored the page at
  vmsim_addr_t block_number = pte >> 1;

  // Get the data from there, and copy it to the real address.
