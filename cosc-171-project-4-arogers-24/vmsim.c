...
swap_out () {
  ...
  // initialize real page address and page table entry
  vmsim_addr_t real_page_addr;
  pt_entry_t pte;

  // initially read lower-level page table entry to address of pte
  vmsim_read_real(&pte, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  // iterate while pte is referenced
  while (IS_REFERENCED(pte)) {    
    // if is referenced, set reference bit to 0
    CLEAR_REFERENCED(pte);
    // write to lower-level page table entry from address of pte
    ...
    // read lower-level page table entry at next hand to address of pte
    vmsim_read_real(&pte, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  }

  // get page address of unreferenced pte
  real_page_addr = GET_PAGE_ADDR(pte);
  // write to backing store
  bs_write(real_page_addr, bs_free_block);

  // bit shift int to ensure most significant bits are 00
  // resident bit and reference bit are 0
  pt_entry_t block = bs_free_block << 2;
  // write to lower-level page table entry of unreferenced pte from bit shifted int
  vmsim_write_real(&block, reverse_page_map[clock_hand], sizeof(pt_entry_t));
  // increment free block in backing store
  bs_free_block++;
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  // read from backing store to real page address
  bs_read(real_page_addr, pte >> 2);
