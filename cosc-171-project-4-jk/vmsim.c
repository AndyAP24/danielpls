...
//static uint64_t	     real_size 	      = 1000;
...
vmsim_addr_t find_victim_lpt_addr(){
 vmsim_addr_t lpt_addr = reverse_page_map[clock_hand];
 pt_entry_t lower_pte = 0;
 vmsim_read_real(&lower_pte,lpt_addr,sizeof(pt_entry_t));
 while(!IS_RESIDENT(lower_pte)||IS_REFERENCED(lower_pte)){
 // printf("Clock_hand %d\n", clock_hand);
  CLEAR_REFERENCED(lower_pte);
  vmsim_write_real(&lower_pte,lpt_addr,sizeof(lower_pte));
  reverse_page_map[clock_hand] = lpt_addr;
  clock_hand = (clock_hand + 1) % normal_real_pages;
  lpt_addr = reverse_page_map[clock_hand];
  vmsim_read_real(&lower_pte,lpt_addr,sizeof(pt_entry_t));
 }
// printf("Chosen_hand %d\n",clock_hand);
 clock_hand = (clock_hand + 1) % normal_real_pages;
 //pte is resident and not referenced;
 return lpt_addr;
}
...
swap_out () {
  ...
  vmsim_addr_t lower_pt_addr = find_victim_lpt_addr();
 // fprintf(stderr,"Swapping out address at 0x%x\n",lower_pt_addr);
  pt_entry_t lower_pte;

  vmsim_read_real(&lower_pte,lower_pt_addr,sizeof(pt_entry_t));
  real_page_addr = GET_PAGE_ADDR(lower_pte);
  bs_write(real_page_addr,bs_free_block);
  lower_pte = (pt_entry_t)(bs_free_block << 4);
 // printf("Block number %d\n", bs_free_block);
 // printf("Retrieved number %d\n", lower_pte >> 4);
  //SET_DIRTY(lower_pte);
  bs_free_block += 1;
  //has a place in the backing store;
  CLEAR_RESIDENT(lower_pte);
  vmsim_write_real(&lower_pte,lower_pt_addr,sizeof(pt_entry_t));
 ...
 * Swap in a page.
 *
 * \param real_page_addr The address of the page frame in real space (RAM) into which to copy a page (from backing store).
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  //APPROACH:
  //Get block number from real_page_addr;
   unsigned int index = (unsigned int) (pte >> 4);
   bs_read(real_page_addr,index);
  // fprintf(stderr,"Swapping in data at block number %d into address 0x%x\n",index,real_page_addr);
} // swap_in ()
// =================================================================================================================================
...
vmsim_map_fault (vmsim_addr_t sim_addr) {
  ...
  if (upper_pte == 0) {
   // fprintf(stderr,"Upper page table non-existent, allocate new %d\n",upper_pte);
  ...
  if (!IS_RESIDENT(lower_pte)) {
   //fprintf(stderr,"Non-resident lower_pte %d\n", lower_pte);
    vmsim_addr_t real_page_addr = allocate_real_page();//Gets you a free page in RAM;
    //If you point to a page that has been swapped out,
    if (lower_pte != 0) {
      //Read page from block in store location  into real_page_addr;
      swap_in(real_page_addr, lower_pte);
    }
    lower_pte = real_page_addr;
    //Denote address as valid;
    SET_RESIDENT(lower_pte);
    //Write new page address into lpt index;
    ...
    //reverse_page_map: Maps block number to location of page in RAM;
