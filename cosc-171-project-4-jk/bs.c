...
//Read block content in disk into buffer(provided address in RAM)
bool
bs_read (vmsim_addr_t buffer, unsigned int block_number) {
  ...
  //OR  Write from block_ptr into buffer
...
//Write from buffer(provided address in RAM) into block in disk;
bool
bs_write (vmsim_addr_t buffer, unsigned int block_number) {
  ...
  // Copy the block into real memory.
  //OR  Read from buffer to block_ptr;
