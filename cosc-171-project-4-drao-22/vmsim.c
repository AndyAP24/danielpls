 ...
 *
 * \param real_page_addr The base address of the real page to set to zero.
 */
void
...
swap_out () {
  ...
  //     get current based on clock
  //     if(current is not used):
  //         victim = current
  //     else:
  //         set current not used
  //     clock_hand++
  // replace victim

  // Pick a page to replace (clock algorithm)
  bool victim_found = false;
  vmsim_addr_t victim_pte_addr;
  pt_entry_t victim_pte;
  while (!victim_found) {

    if (debug >= 2) {
      printf("\nBegin clock algorithm cycle, clock hand at page %d\n", clock_hand);
    }
    
    // Get the current PTE
    vmsim_addr_t current_pte_addr = reverse_page_map[clock_hand];
    pt_entry_t current_pte;
    vmsim_read_real(&current_pte, current_pte_addr, sizeof(pt_entry_t));

    if (debug >= 2) {
      printf("current_pte_addr: %x,\tcurrent_pte: %x\n", current_pte_addr, current_pte);
    }
    ...
    if (!IS_REFERENCED(current_pte)) {
      ...
      // Victim found
      victim_found = true;
      victim_pte_addr = current_pte_addr;
      victim_pte = current_pte;
      if (debug >= 2) {
	printf("Found unused page. PTE: %x\n", victim_pte);
      }
      ...
      // Clear the referenced bit for the next time around
      if (debug >= 2) {
	printf("Current page in use. Old PTE: %x. ", current_pte);
      }
      CLEAR_REFERENCED(current_pte);
      if (debug >= 2) {
	printf("New PTE: %x\n", current_pte);
      }
      vmsim_write_real(&current_pte, current_pte_addr, sizeof(pt_entry_t));

    }

    // Cycle the clock
    clock_hand++;
    if (clock_hand == normal_real_pages) {
      if (debug >= 2) {
	printf("\nWrapping clock around\n");
      }
      clock_hand = 0;
    } 
  ...
  if (debug >= 2) {
    printf("\nClock algorithm complete\n");
    printf("Victim PTE: %x\n", victim_pte);
  }

  // If chosen page is dirty, write it back to backing store
  if (IS_DIRTY(victim_pte)) {

    // Create a new PTE that stores the block number in the backing storage
    pt_entry_t new_pte = bs_free_block;
    // Shift the block number to have the flag bits (all cleared)
    new_pte = new_pte << 3;
    // Update the page table with the new entry
    vmsim_write_real(&new_pte, victim_pte_addr, sizeof(pt_entry_t));

    if (debug >= 2) {
      printf("Victim page is dirty.\n");
      printf("Writing to backing store block number: %d\n", bs_free_block);
    }

    // Write to backing store and update its free pointer
    ...
    if (debug >= 2) {
      printf("Victim page not dirty\n");
    }
    // There also needs to be some way to update the PTE to indicate the (old) backing
    // store block number, but that's not preserved since the PTE was updated when we
    // loaded this block in. So how to find that?
    // Maybe we just need a separate data structure for storing associated block
    // numbers? Instead of storing block numbers in PTE when we swap out. Then we only
    // need to update that table on IS_DIRTY, otherwise leave the old number. And just
    // set the PTE to not resident.
    // Seems potentially wasteful though, since for all blocks either PTE or block
    // number table would be out-of-date. Also would probably require work in init()
    // function to create the new table so I don't think that's the expected or ideal
    // solution.
    
  }

  // Return base address of real page
  real_page_addr = GET_PAGE_ADDR(victim_pte);
  if (debug >= 2) {
    printf("\nReturning real page address: %x\n", real_page_addr);
  }
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  // real_page_addr is already cleared during allocation
  
  // Copy bytes from backing storage into here
  // Backing store block number is in the upper 29b of the PTE
  unsigned int bs_block_num = pte >> 3;
  if (debug >= 2) {
    printf("Reading from backing store block number %d\n", bs_block_num);
  }
  // Execute read
  bs_read(real_page_addr, bs_block_num);
  // PTE gets updated in vmsim_map_fault()
