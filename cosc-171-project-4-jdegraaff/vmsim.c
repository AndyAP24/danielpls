...
swap_out () {
  ...
  /*
  Use reverse_page_map to go through each page table entry: loop back after looking at all the entries (normal_real_pages)
  If the referenced bit is 1, set it to 0 and continue
  If the referenced bit is 0, this is the page we're swapping out:
  Back the page up in backing storage (bs_write(real_page_addr, bs_free_block))
  Write the (formerly free) bs_free_block into the PTE, plus 0 at the beginning for resident bit
  Then return the selected page's address
  */

  vmsim_addr_t real_page_addr;
  vmsim_read_real(&real_page_addr, (reverse_page_map + clock_hand), sizeof(vmsim_addr_t)); //this should write the next entry from reverse_page_map into real_page_addr
  while(IS_REFERENCED(real_page_addr)) {
    
    //if it's already referenced, dereference it and update the reverse_page_map
    CLEAR_REFERENCED(real_page_addr);
    vmsim_write_real(&real_page_addr, (reverse_page_map + clock_hand), sizeof(vmsim_addr_t));

    clock_hand += sizeof(vmsim_addr_t); //increment clock hand to look at next entry in reverse_page_map
    if(clock_hand == sizeof(vmsim_addr_t) * normal_real_pages) clock_hand = 0; //loop back when clock hand gets to all entries (normal_real_pages)

    vmsim_read_real(&real_page_addr, (reverse_page_map + clock_hand), sizeof(vmsim_addr_t)); //go to the next page address
  } //by the time we get here, real_page_addr should be an unreferenced PTE: this is the page we're swapping out

  bs_write(GET_PAGE_ADDR(real_page_addr), bs_free_block); //write this page to backing storage
  unsigned int temp = bs_free_block << 1;
  vmsim_write_real(&temp, (reverse_page_map + clock_hand), sizeof(vmsim_addr_t)); //change the address to bs_free_block so that I can find it later, <<1 because its resident bit is 0
  bs_free_block++;
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  /*
  Call bs_read(), passing in pte as block_number and real_page_addr as the buffer to write the data into
  Update the pte
  */

  bs_read(real_page_addr, (pte >> 1)); //lope off the resident bit so that pte = block where it's located, and then put that data in real_page_addr
  
  vmsim_write_real(&pte, real_page_addr, sizeof(real_page_addr));
  SET_RESIDENT(pte);
  pte |= 0x2; //set_referenced
