...
// =================================================================================================================================
/**
 * Swap out some page. Trying to find victim page based on clock algorithm
 *
...
// COMPLETE ME:  Select a real page to swap out to the backing store, and then return the base address of that real page.
  ...
  printf("Swap out was accessed \n");

  //iterate until victim page is found
  while(!victim_found){
    //indexing into array that holds pointers to lower page table entries
    //the clock_hand allows for more than one iteration over the entire array trying to look for a victim page  
    //clock_hand corresponds to the real_page_number
    vmsim_addr_t lpte_addr = reverse_page_map[clock_hand];
    pt_entry_t current_lpte;
    pt_entry_t victim_lpte;
    vmsim_read_real(&current_lpte, lpte_addr, sizeof(current_lpte));

    if(IS_RESIDENT(current_lpte)){
      //if the used/referenced bit is 1, this means it has been recently referenced, so it is not the victim page
      if(IS_REFERENCED(current_lpte)){
        //since we have gone over this entry now, we can clear the referenced bit and reset to 0, so it can be cleared for the 
        //next revolution in the clock array
        CLEAR_REFERENCED(current_lpte);
        vmsim_write_real(&current_lpte, lpte_addr, sizeof(current_lpte));
      }
      //if the referenced bit is 0, this is our victim page
      else{
        victim_lpte = current_lpte;
        
        //getting page address from the page table entry
        real_page_addr = GET_PAGE_ADDR(victim_lpte);

        //Since we've cleared the resident bit, by doing the below operation, the shift operator will shift the pte so that
        //the last bit will hold the resident bit = 0
        //this stores the block number, which shouldn't take up more than 31 bits, while still storing the resident bit in 
        //position 0, which only takes up 1 bit
        victim_lpte = bs_free_block << 1;
        //since we are swapping this page out, it's entry no longer has a resident bit = 1
        //we have now confirmed the victim page is no longer valid (in regards to translation between simulated and real address)
        CLEAR_RESIDENT(victim_lpte);

        vmsim_write_real(&victim_lpte, lpte_addr, sizeof(victim_lpte));

        //writing the page into the backing store
        bs_write(GET_PAGE_ADDR(current_lpte), bs_free_block);

        //incrementing starting of free block in the backing store
        bs_free_block++;

        victim_found = true;
      }
    }
    //how to iterate through reverse_page_map array in a clock-algorithm method
    clock_hand = (clock_hand + 1) % sizeof(reverse_page_map);
    printf("clock hand value: %d\n", clock_hand);
  }
  printf("swap out result: %d\n", real_page_addr);
  //returning the real page address whose contents where copied into the backing store
 ...
 * \param pte The page table entry of the simulated page to swap in.
 ...
 - pte has resident = 0
 - find block number
 - when i did swap out, i had a block number
 - since pte is not valid anymore, the value of pte, which is the page, the page's addresses are irrelevant
 - use the page's 32 bits for address to store the block number (during swap out)
 */
void
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  printf("Swap in was accessed \n");

  //getting the block number stored in the page table entry
  unsigned int block_number = pte >> 1;

  printf("block number: %d\n", block_number);

  //reading page from the backing store and placing into the given address in real memory
  bs_read(real_page_addr, block_number);

  //copying the block page that was placed in real memory into the page table entry
  vmsim_read_real(&pte, real_page_addr, sizeof(pte));

  printf("swap in real_page_addr: %d\n", real_page_addr);

  //since we are placing this block into real memory, it's resident/valid bit should = 1
  SET_RESIDENT(pte);
...
/*
  Since the clock_hand is an unsigned int, and will serve as the index in the clock array, we can equate the 
  clock_hand to the real_page_number which is the index into the reverse_page_map array. This equates the clock array
  to the reverse_page_map array.
  */

/*

    //checking if the page is valid by checking the resident bit
    //if the page is not valid, then simulated address is not mapped to real page, so page fault is triggered
    if(!IS_RESIDENT(reverse_page_map[clock_hand])){
      vmsim_map_fault(reverse_page_map[clock_hand]);
    }
    //if the page is valid, then check the used bit
    else{
      //if the used bit is 1, meaning it has been referenced
      if(IS_REFERENCED(reverse_page_map[clock_hand])){
        //reset the used bit to 0
        CLEAR_REFERENCED(reverse_page_map[clock_hand]);
        //move on to next item in the clock array
        //since it is a clock array, if the end of the array is reached, we want to restart and start from index 0
        clock_hand = (clock_hand + 1) % sizeof(reverse_page_map);
      }
      //if the used bit is 0, meaning it has not been referenced, victim page is found
      else{
        if(!IS_REFERENCED(reverse_page_map[clock_hand])){
        //indexing into array and finding the lower page table entry that correlates with the specific real_page_number = clock_hand
        lower_pte_addr = reverse_page_map[clock_hand];
        pt_entry_t   lower_pte;
        vmsim_read_real(&lower_pte, lower_pte_addr, sizeof(lower_pte));

        vmsim_addr_t victim_real_page = GET_PAGE_ADDR(lower_pte);
        //combining real page address plus offset to give the real_address
        vmsim_addr_t real_address = real_page + offset;
        vmsim_addr_t victim_real_page = GET_PAGE_ADDR(victim_pte);
      }
      }
    }
      // Add the PTE addr to the reverse map for this page.
  ...
  //use the clock method here
  //to pick the right real page, i have to choose a victim page that has its used bit set to 0
  //use reverse_page_map to do the clock method and find the victim page
  //once victim page is found, if dirty, copy contents into hard disk and 
  //change values in array to reflect PTE
