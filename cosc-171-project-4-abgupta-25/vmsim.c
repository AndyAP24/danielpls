...
swap_out () {
  ...
  //read a page, if referenced, clear the bit and swap out, if unreferenced, then make sure it is no longer resident, write to bs and ready for swap out

  while(true){

    //printf("starting swap out\n");

    pt_entry_t pg;//create a temp variable as we iterate through pages

    pt_entry_t pgSwap;//create a var for the page to swap out

    vmsim_addr_t pageEntry = reverse_page_map[clock_hand]; //grab the address of the page we are looking at

    vmsim_read_real(&pg, pageEntry, sizeof(pg));//read the page table entry that we are at

    if(IS_REFERENCED(pg)) {

      //printf("already referenced block\n");

      CLEAR_REFERENCED(pg);//clear out the referenced bit

      vmsim_write_real(&pg, pageEntry, sizeof(pg));//write it back in with the address changed with new referenced bit
      
    }//run if the page that is read is referenced
    else {

      //printf("unreferenced block\n");

      pgSwap = bs_free_block << 1;//shift by 1

      CLEAR_RESIDENT(pgSwap);//clear out the resident bit of the page that we want to swap

      vmsim_write_real(&pgSwap, pageEntry, sizeof(pgSwap));//write the new address to to the pageEntry (which goes into the reverse map)

      real_page_addr = GET_PAGE_ADDR(pgSwap);//get the real address of the page to swap out

      bs_write(real_page_addr, bs_free_block);//write our swapped block into the backing storage

      bs_free_block++;//increment the free block for next time

      return real_page_addr;//return the address

      break;//break the while loop
      
      
    }//if it not referenced, this is the page we want to swap

    //printf("Loop over\n");

    clock_hand = (clock_hand + 1) % normal_real_pages;//move the clock hand along
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  //printf("running swap in\n");
  unsigned int block = pte >> 1;//grab the block to swap in
  bs_read(real_page_addr, block);// read the page from the backing store into the real page
