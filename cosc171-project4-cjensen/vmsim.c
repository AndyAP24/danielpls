...
swap_out () {
  ...
  vmsim_read_real(&cur, reverse_page_map[clock_hand], sizeof(cur));

  // clock algorithm
  // if clock hand references a page that hasn't been referenced
  while (IS_REFERENCED(cur)) {
    CLEAR_REFERENCED(cur);

    vmsim_write_real(&cur, reverse_page_map[clock_hand], sizeof(cur));

    // cycle clock hand
    // check if clock hand[i + 1] is out of bounds
    if (clock_hand + 1 == normal_real_pages) {
      clock_hand = 0;
    } else {
      clock_hand += 1;
    }
    // read the current index of newly incremented clock_hand
    vmsim_read_real(&cur, reverse_page_map[clock_hand], sizeof(cur));
  }

  // fill in empty space with new, blank page table entry
  pt_entry_t new_block = bs_free_block << 2;
  vmsim_write_real(&new_block, reverse_page_map[clock_hand], sizeof(new_block));

  // write the victim page to the back storage at bs_free_block index
  real_addr = GET_PAGE_ADDR(cur);
  bs_write(real_addr, bs_free_block);
  bs_free_block += 1;
...
swap_in (vmsim_addr_t real_page_addr, pt_entry_t pte) {
  ...
  // read from backing store into real_page_addr
  // need to remember what number was put in (not clock_hand)
  pte = pte >> 2;
